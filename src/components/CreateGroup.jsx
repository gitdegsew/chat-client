...
const CreateGroup = ({ setIsHiddden, groups, setGroups }) => {
  ...
  const { setChats, chats, setMessageToSend, setChatSelected } =
  ...
  const handelCreateGroup = () => {
    ...
    setIsLoading(true);
    postGroup(user.accessToken, groupName, user.id)
      .then((result) => {
        setIsLoading(false);
        setGroup(result);
        setIsHiddden(true);
  ...
  return (
    <div
      id="createGroup"
      className="absolute top-[10%] left-[30%] rounded-xl z-50 w-[40%] h-56 bg-[#003327]  flex flex-col justify-around items-center"
    ...
    >
      ...
      {!isLoading && (
        <>
          <h3 className="text-white font-Roboto text-3xl">Enter group name</h3>
      ...
      <div className="flex justify-around items-center gap-8 font-slab text-lg  font-bold pb-5 ">
        <span
          className="text-[hsl(180,46%,92%)] cursor-pointer hover:text-[#5dd75d]"
          onClick={handleCandcelGroup}
        >
          cancel
        </span>
        <span
          className={`text-[hsl(180,46%,92%)]  ${
            !error && groupName
              ? "cursor-pointer hover:text-[#8d686c]"
              : "text-gray-300"
          } `}
          onClick={error ? () => {} : handelCreateGroup}
        >
          create
        </span>
      </div>
