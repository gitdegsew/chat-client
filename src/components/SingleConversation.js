...
const SingleConversation = ({ chat, chatSelected, setChats }) => {
  ...
  // <CircularProgressbar value={percentageT} className='w-8 h-8' />
  return (
    <div
      className={`flex my-2 gap-3 ${isUser && chat.sender !=="ADMIN" ? "flex-row-reverse" : "flex-row"} ${chat.sender==="ADMIN"&& "flex justify-center items-center text-center "}`}
    >
      {name && chat.sender !=="ADMIN" &&
        <div className="flex flex-col justify-center w-12 h-12 rounded-full bg-slate-800
      ...
      <div
        ...
        } ${chat.sender ==="ADMIN" && "bg-[rgb(240,248,255)] h-8 text-[#008080]"} p-2  justify-between gap-y-4'`}
      >
        ...
        chat.messageType === "video-db" ? (
         ...
         chat.messageType === "non-text" ? (
          ...
          <p>{chat.sender==="ADMIN" && chat.message.split(' ')[0]===user.username? `You  ${chat.message.split(' ')[2]} the group` :chat.message}</p>
        )}
        {chat.sender !=="ADMIN" && <p className="text-xs text-[#354a40]">{`${htd}:${mtd} ${identify}`}</p>}
