...
const MyVideos = ({ isVidEnabled, userToCall, isLoading }) => {
  ...
  return (
    <div
      className={`${
        !isAnswered && "flex  justify-center items-center w-full  gap-x-14    "
      }`}
    >
      <div
      ...
      >
        <video
          autoPlay
          ref={myVideo}
          className={`p-3 w-full h-72  bg-slate-800 ${
            isAnswered && "rounded-2xl z-40"
          }  `}
          hidden={!isVidEnabled}
          muted
        />
        ...
        {!isVidEnabled && (
          <div
            className={`p-3  flex flex-col justify-center w-full h-full    bg-slate-800 text-white  items-center ${
              isAnswered &&
              "border-solid border-2 border-sky-500 mt-3 bg-zinc-50 rounded-lg w-48 h-64"
            } `}
          >
            <div
              className={`bg-red-500 rounded-full flex justify-center items-center ${
                isAnswered ? "w-24 h-24" : "w-44 h-44"
              }  `}
            >
              <p className={`${!isAnswered ? "text-9xl" : "text-5xl"}`}>
      ...
      >
        ...
        {!isAnswered && (
          <div className="p-8 flex flex-col justify-center w-full h-full mt-[-60]  bg-slate-800 text-white  items-center">
